...
def generator1m(n, u, c, ep, D):
    ...
    for j in range(c):
        for m in range(n):
            coeffients1 = np.random.rand(len(C[j]))
            coeffients2 = np.random.rand(n)
            for i in C[j]:
                for l in range(n):
                    G[u * n + j * n + m, (u - i) * n + l] = coeffients1[i - 1] * coeffients2[l]
    # print(np.count_nonzero(G))
    return G, np.mean([len(x) for x in C]) * n

def test_failure1m(n, u, c, ep, D):
    G, _ = generator1m(n, u, c, ep, D)
...
def test_failure_random1m(n, u, c, ep, times, D):
    G, C = generator1m(n, u, c, ep, D)
...
def test1m():
    ...
    for i in range(0, step + 1):
        ep = i / step
        # ep = 0.1
        # test1 = test_failure1m(n, u, c, ep, D)
        test2, C = test_failure_random1m(n, u, c, ep, times, D)
        # if test2 < ep:
        #     print(ep, test2)
